...
> ## Exercise
>
> Create two objects `length` and `width` and assign them values. *Note: Because the term "width" is also the name of a function, RStudio might add parenthesess "()", which would be incorrect and cause unexpected results. Some programmers will use abbreviations for their object names to avoid this issue.*
> Create an object `area` that is calculated from the `length` and `width` object values (e.g., multiply) and print the value of `area`.
> Set the values of `length` and `width` to different numbers and print the value of `area` again. 
> Did the value of `area` change, too?
> 
> 
> > ## Solution
> > 
