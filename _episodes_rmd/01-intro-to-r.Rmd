...
- "What is an object?"
- "How can values be initially assigned to objects of different data types?"
...
> have drastically different meanings, even if they are sometimes used synonymously. 
> Because we will be mostly using objects that represent data tables, in which 
> a 'variable' is not an 'object', we will continue to use the term object here. 
...
> ## Exercise
>
> Create two objects `length` and `width` and assign them values. Note: Because the term "width" is also the name of a function, RStudio might add parenthesess "()", which would be incorrect and cause unexpected results. Some programmers thus abbreviate common words to avoid this issue.
> Create a third object `area` and give it a value based on the current values of `length` and `width`.
...
confused with the single `=` sign, which performs object assignment (similar
to `<-`).
